...
export default function LineChart({
  ...
  return (
    <div ref={wrap} className="relative">
      {!hasData ? (
        <div className="flex items-center justify-center rounded-md border border-dashed border-border px-4 text-center" style={{ height: H }}>
          <p className="font-mono text-fluid-xs text-muted">{empty}</p>
        </div>
      ) : (
        <>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-1 pb-1 font-mono text-fluid-xs text-muted">
          ...
          <svg ref={ref} viewBox={`0 0 ${W} ${H}`} height={H} className="block w-full select-none touch-none" role="img" aria-labelledby={id}
          ...
          {hover !== null && (
            <div className="pointer-events-none absolute top-6 rounded-md border border-accent/25 bg-surface px-3 py-2 shadow-card font-mono text-fluid-xs"
